...
namespace SDKTemplate
{
    ...
    public partial class DataHelpers
    {
        ...
        public static string ConvertAsciiToDecimal(string AsciiString)
        {
            string DecimalString = string.Empty;
            for(int i=0;i< AsciiString.Length;i+=2)
            {
                string s = AsciiString.Substring(i, 2);
                char temp = (char)Convert.ToInt32(s,16);
                char ch = (char)temp;
                DecimalString += temp.ToString();
            }
        ...
        public static string GetDataLabelString(IBuffer data, uint scanDataType)
        {
            ...
            string DecimalString = ConvertAsciiToDecimal(DataHelpers.GetDataString(data));
            return string.Format("Decoded data unavailable. Raw label data: {0}", DecimalString);
            //return CryptographicBuffer.ConvertBinaryToString(BinaryStringEncoding.Utf8, data);
        }
    }
